...
The relevant pulldown-cmark events for math. In both cases, the `latex` variable contains the LaTeX content in the math sections, and the `mathml` variable is the corresponding MathML generated by pulldown-latex. The MathML is wrapped and emitted as an event.

```rust
pub(crate) struct CustomIterator<'a, I: Iterator<Item = Event<'a>>> {
    storage: pulldown_latex::Storage,
    // ..
}
...
The function that converts LaTeX to MathML.

```rust
pub(crate) fn latex_to_mathml(
    latex: &str,
    storage: &mut pulldown_latex::Storage,
    display_mode: pulldown_latex::DisplayMode,
) -> Result<String> {
...
The relevant pulldown-cmark events. Two things are set up when a code block begins: an empty `captive_string` to collect the block’s contents, and an optional `syntax_tag` to identify the code block's language. 

For fenced blocks with a language, `syntax_tag` is set to that language. For indented blocks or fenced blocks without a language, it’s `None`. As text events within the code block is appended to `captive_string`. When the matching code block end is reached, the collected code is finalized. If `syntax_tag` was set to a language, it’s passed with the code to the highlighter to produce HTML. Otherwise, the code is left unhighlighted.
...
The function that highlights code.
